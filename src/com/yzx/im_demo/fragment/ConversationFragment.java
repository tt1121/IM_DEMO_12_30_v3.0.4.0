...
public class ConversationFragment extends Fragment implements
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		((IMChatActivity) getActivity()).putObserver(this);
		initdata();
	}
	...
	public Handler mHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case 2:
				...
				break;
			case 3:
				ConversationInfo cinfo = (ConversationInfo) msg.obj;
				addConvertInfo(cinfo);
				sendEmptyMessage(1);
				break;
			case 4:
				...
				break;
			case 5:
				ConversationInfo info = (ConversationInfo) msg.obj;
				ConversationInfo myInfo = getInfo(info);
				if (myInfo != null) {
					updataCinfo(info, myInfo);
				}
				sendEmptyMessage(1);
				break;
			case 6:
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		Log.i(TAG, "onActivityCreated hashCode = " + hashCode());
		super.onActivityCreated(savedInstanceState);
	}

	@Override
	public void onDestroyView() {
		Log.i(TAG, "onDestroyView contentView = null ? " + (mView == null));
		isDestoryView = true;
		super.onDestroyView();
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		isDestoryView = false;
		mView = inflater.inflate(R.layout.activity_conversationlist, container,
				false);
	...
	class ConversationlistAdapter extends BaseAdapter {
		...
		public View getView(final int position, View convertView,
			...
			DraftMsg df = DraftDbManager.getInstance().getByTargetId(cinfo.getTargetId());;
			...
			String time = DateTools.formatDate(format.format(new Date(cinfo.getLastTime())),false);
			viewHolder.conversation_time.setText(time);
	...
	private void updataCinfo(ConversationInfo cinfoSrc,
		...
		synchronized (conversationLists) {
			...
			if (cinfoSrc.getCategoryId() == CategoryId.DISCUSSION) {
				...
				if(mgs!= null && mgs.size() > 0 && !TextUtils.isEmpty(mgs.get(0).getNickName())
					...
					Log.i(TAG, "消息更新时，title = "+cinfoDest.getConversationTitle());
					Log.i(TAG, "消息更新时，SenderId = "+mgs.get(0).getSenderId());
	...
	public void onDeleteConversation(ConversationInfo cinfo) {
		Log.i(TAG,"onDeleteConversation title : " + cinfo.getConversationTitle()+ ",targetId : " + cinfo.getTargetId());
		Message message = mHandler.obtainMessage();
		message.obj = cinfo;
		message.what = 4;
		mHandler.sendMessage(message);
	...
	public void onUpdateConversation(ConversationInfo cinfo) {
		Log.i(TAG,
				"onUpdateConversation title : " + cinfo.getConversationTitle()
						+ ",targetId : " + cinfo.getTargetId());
		if (cinfo.getCategoryId() == CategoryId.DISCUSSION) {
			DiscussionInfo info = IMManager.getInstance(getActivity())
					.getDiscussionInfo(cinfo.getTargetId());
			if (info != null) {
				cinfo.setConversationTitle(info.getDiscussionName());
			}
		}
		Message message = mHandler.obtainMessage();
		message.what = 5;
